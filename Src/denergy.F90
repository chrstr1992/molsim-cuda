...
!   write(uout,*) txroutine
  ...
  400 continue
   ...
   !if (tidx == 1) then
   !do i =1, nptpt_d
   !     utwobnew_d(0) = utwobnew_d(0) + utwobnew_d(i)
   !     dutwob_d(i) = utwobnew_d(i)
      !  print *, "utwobnew: ", i, utwobnew_d(i)
   !end do
   !end if
   !dutwob_d(0) = utwobnew_d(0)
     !   print *, "lhsover: ", lhsoverlap
...
!  ! write(uout,*) txroutine
  ...
  400 continue
       ...
       if (tidx_int == 1) then
          do i = 1, blockDim%x
            usum_aux2(iptjpt_arr(i)) = usum_aux2(iptjpt_arr(i)) + usum(i)
          end do
          do i = 1, nptpt_d
             istat = atomicAdd(utwobold_d(i),usum_aux2(i))
          end do
       end if
