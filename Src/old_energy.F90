...
subroutine UTwoBodyA
   ...
   do iploc = 1, Getnpmyid()
      ...
      do jploc = 1, nneighpn(iploc)
         ...
         dy = ro(2,ip)-ro(2,jp)
         dz = ro(3,ip)-ro(3,jp)
         istat = cudaMemcpy(dx_d,dx,1)
         istat = cudaMemcpy(dy_d,dy,1)
         istat = cudaMemcpy(dz_d,dz,1)
         istat = cudaMemcpy(r2_d,r2,1)
         call PBCr2_cuda<<<1,1>>>(dx_d,dy_d,dz_d,r2_d)
         istat = cudaMemcpy(r2,r2_d,1)
         if (r2 > rcut2) cycle
